...
df = pd.DataFrame(columns=['address', 'price', 'property type', 'building size', 'year built', 'building class', 'lat', 'lon']) 
...
content, soup, ct = parse_url("https://www.loopnet.com/search/office-buildings/brooklyn-ny/for-sale/?sk=1c436c3499898580bdacd4e393ed2494&bb=2k3mttjg3Hul19i8pE") 
...
if len(results) > 0: 
        with open('data_scrapped.csv', 'w', newline = '') as f:
            ...
            for r in results:   
                ...
                try:
                    att_price = int(row_results['Price'][1:])
                except:
                    att_price = np.nan
                try:
                ...
                try:
                    att_building_size = row_results['Building Size']
                except:
                    att_building_size = np.nan
                try:
                    att_year_built = int(row_results['Year Built'])
                except:
